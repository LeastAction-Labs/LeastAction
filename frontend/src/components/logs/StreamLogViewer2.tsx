...
export default function StreamLogViewer({
  ...
  const buildUrl = useCallback(
    (skip: number) => {
      ...
      try {
        // Use standard URL parsing to gracefully append variables without text collision
        const urlObj = new URL(base);
        urlObj.searchParams.set('reverse', 'true');
        urlObj.searchParams.set('skip', skip.toString());
        urlObj.searchParams.set('limit', pageSize.toString());
        return urlObj.toString();
      } catch {
        // Direct concatenation fallback for relative roots if window.location isn't implied
  ...
  const filteredLines = useMemo(() => {
    return parsedLines.filter(({ inheritedLevel, raw }) => {
