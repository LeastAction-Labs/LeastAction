...
export default function LookerViewer({ item }: Props) {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      // Looker URLs are one-time-use — always re-fetch on load, never reuse
      const t = await getEmbedToken(item.laui);
      setToken(t);
      if (timerRef.current) clearTimeout(timerRef.current);
      timerRef.current = setTimeout(
        () => void load(),
        Math.max((t.expires_in - 30) * 1000, 10_000),
      );
    } catch (e: any) {
      setError(e?.response?.data?.detail ?? e?.message ?? 'Failed to load Looker embed.');
    }
  }, [item.laui]);
