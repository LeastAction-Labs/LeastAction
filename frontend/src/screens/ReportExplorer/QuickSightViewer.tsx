...
export default function QuickSightViewer({ item }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setToken(t);
      // 600-min session; refresh 5 min before expiry
      if (timerRef.current) clearTimeout(timerRef.current);
      timerRef.current = setTimeout(
        () => void load(),
        Math.max((t.expires_in - 300) * 1000, 60_000),
      );
    } catch (e: any) {
      setError(e?.response?.data?.detail ?? e?.message ?? 'Failed to load QuickSight embed.');
    }
  }, [item.laui]);
