...
export const Route = createFileRoute('/public/marketplace-system-callback')({
...
function RouteComponent() {
    ...
    const callback = async () => {
      ...
      if (localStorage.getItem('system_marketplace_auth_processing')) {
        console.log('Marketplace auth already processing, skipping duplicate request')
        return 
      }
  
      if (localStorage.getItem('system_marketplace_auth_started')) {
        localStorage.setItem('system_marketplace_auth_processing', 'true')
  
        const queryString = window.location.search
        const params = new URLSearchParams(queryString)
        const stateFromQuery = params.get('state')
        const codeFromQuery = params.get('code')
        const stateFromLocalStorage = localStorage.getItem('system_marketplace_auth_state')
        if (stateFromLocalStorage?.includes(stateFromQuery||"") && codeFromQuery) {
          try {
            ...
            localStorage.removeItem('system_marketplace_auth_started')
            localStorage.removeItem('system_marketplace_auth_state')
            localStorage.removeItem('system_marketplace_auth_processing')
