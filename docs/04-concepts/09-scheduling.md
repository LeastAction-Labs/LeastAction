...
|---|---|---|
| `logical_date` | The **data epoch** the task processes — injected as `{{ds}}` / `{{logical_date}}` in the payload, and determines the log storage path. Floored to the cron's granularity (daily → midnight, monthly → 1st, yearly → Jan 1, sub-hourly → exact minute). | one cron tick forward |
