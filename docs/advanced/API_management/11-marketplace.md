...
The user must have a marketplace access token stored. Use [POST /user/marketplace/token](/path?laui=getting-started-advanced-API_management-10-user&itemtype=doc.file&itemname=10%20User#post-apiv1usermarketplacetoken) to set one.
...
The imported item's `item_type` is preserved from the Marketplace source. The `parent_laui` must point to a folder that supports the item type (see [Catalog Hierarchy](/path?laui=getting-started-advanced-API_management-schemas-catalog-hierarchy&itemtype=doc.file&itemname=Catalog%20Hierarchy)).
