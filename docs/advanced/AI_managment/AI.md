...
See [Operator Guide](/path?laui=getting-started-advanced-task_managment-operator&itemtype=doc.file&itemname=Operator) for the full method contract and examples.
...
See [Action Guide](/path?laui=getting-started-advanced-task_managment-action_aka_hook&itemtype=doc.file&itemname=Action%20Aka%20Hook) for lifecycle types, configuration, and the full list of built-in actions.
