...
- `LeastActionRerunSubtree` — rerun a failed task and every downstream task that depends on it, in one click
- `LeastActionSkipSubtree` — skip a task and its entire downstream subtree when you need to bypass a branch
- `LeastActionCancel`, `LeastActionSkip`, `LeastActionRun`, `LeastActionRerun` — available per-workflow, filtered by task status so only valid operations show
...
LeastAction's `LeastActionGitSync` action can pull code from the same Git repositories your Airflow pipelines use, so payload code can be shared without duplication.
