...
def run(least_action_task_object, client):
    try:
        ...
        if "configuration_overrides" in payload:
            params["configurationOverrides"] = payload["configuration_overrides"]
        ...
        log_info("task", "run", "submitting_job",
                 f"Submitting job '{name}' to virtual cluster: {virtual_cluster_id}")

        response = client.start_job_run(**params)
        job_run_id = response.get("id")

        log_info("task", "run", "job_submitted",
                 f"Job run {job_run_id} submitted, polling for COMPLETED state...")

        terminal_states = {"COMPLETED", "FAILED", "CANCEL_PENDING", "CANCELLED"}
        while True:
            desc = client.describe_job_run(virtualClusterId=virtual_cluster_id, id=job_run_id)
            state = desc.get("jobRun", {}).get("state")
        ...
        log_info("task", "run", "job_complete",
                 f"Job {job_run_id} completed successfully on cluster {virtual_cluster_id}")
...
def check_completion(least_action_task_object, client, run_details):
    log_info("task", "check_completion", "sync_complete",
             "EMR ContainerSubmitJob is synchronous - already complete")
    return {
        "status": run_details.get("status", "success"),
        "message": "EMR on EKS job completed",
    ...
    try:
        ...
        if status == "success":
            ...
            log_info("task", "finish", "job_summary",
                     f"Job {output.get('job_run_id')} on cluster {output.get('virtual_cluster_id')} "
...
payload = {
    ...
    "job_driver": {
        "sparkSubmitJobDriver": {
            "entryPoint": "s3://my-bucket/app.py",
            "sparkSubmitParameters": "--conf spark.executor.instances=2 --conf spark.executor.cores=2"
        }
    }
}

prompt = (
    "Create an operator that submits a Spark job to an Amazon EMR on EKS virtual cluster. "
    "Required payload fields: virtual_cluster_id, name, execution_role_arn, release_label, job_driver. "
    "Optional: configuration_overrides, client_token, tags (dict). "
    "Auth: try IAM role via STS first; if unavailable, fall back to access keys from connection. "
    "After calling start_job_run, poll describe_job_run every 15 seconds until state is COMPLETED "
    "or a terminal failure state (FAILED, CANCEL_PENDING, CANCELLED). "
    "Return job_run_id, virtual_cluster_id, and state=COMPLETED on success. "
    "Catch all AWS errors and return them as status:failed - never raise."
)

install_docs = """# AWSEMRContainerSubmitJob - Install Guide
...
- An existing EMR on EKS virtual cluster (use AWSEMREksCreateCluster to create one)
- An IAM execution role with permissions to run Spark jobs and access S3
...
guide_docs = """# AWSEMRContainerSubmitJob - Operator Guide

## What it does

Submits a Spark job to an Amazon EMR on EKS virtual cluster and polls synchronously every
15 seconds until the job reaches COMPLETED or a terminal failure state. The job runs as a
Kubernetes pod in the specified EKS cluster.
...
|---------------------|----------|----------------------------------------------------------|
| virtual_cluster_id  | Yes      | ID of the EMR on EKS virtual cluster                     |
| name                | Yes      | Display name for the job run                             |
| execution_role_arn  | Yes      | IAM role ARN for job execution                           |
| release_label       | Yes      | EMR release label (e.g. emr-6.15.0-latest)              |
| job_driver          | Yes      | Job driver config with sparkSubmitJobDriver              |
| configuration_overrides | No  | Optional Spark/monitoring configuration overrides        |
...
- Does not create the virtual cluster (use AWSEMREksCreateCluster)
- Does not handle job cancellation
- Does not retrieve job logs (access via CloudWatch or S3 log URI)
"""

description = """
Submits a Spark job to an Amazon EMR on EKS virtual cluster and polls synchronously until
COMPLETED or a terminal failure state. Uses the emr-containers StartJobRun API and polls
describe_job_run every 15 seconds. Requires an existing virtual cluster and an IAM execution
role. Auth: IAM role via STS first, fallback to access keys. Returns job_run_id,
virtual_cluster_id, and state=COMPLETED on success.
"""
...
metadata = {
    "service": "EMR",
    "category": "BigData",
    "tags": ["emr", "eks", "kubernetes", "spark", "job", "aws"],
    "airflow_equivalent": "EmrContainerOperator"
}
...
`job_driver` must specify `sparkSubmitJobDriver` or `hiveJobDriver`. The execution role needs S3, CloudWatch Logs, and any other service permissions the job requires. Configure `monitoringConfiguration.s3MonitoringConfiguration` in `configurationOverrides` to persist job logs to S3.
"""
