...
def run(least_action_task_object, client):
    try:
        ...
        if current_status == "available":
            log_info("task", "run", "already_available",
                     f"Cluster '{cluster_id}' is already available, skipping resume")
            return {
        ...
        log_info("task", "run", "resuming_cluster",
                 f"Resuming Redshift cluster '{cluster_id}'")
        ...
        while True:
            ...
            if status not in {"resuming", "available"}:
                return {
                    "execution_type": "async",
                    "status": "failed",
                    "result": {
    ...
    try:
        if run_details.get("status") in ("success", "failed"):
            return {
                ...
                "message": "Redshift cluster resume completed",
        ...
        log_info("task", "check_completion", "polling_status",
                 f"Cluster '{cluster_id}' status: {status}")
        if status == "available":
            return {"status": "success", "message": "Cluster is available",
                    "output": {"cluster_identifier": cluster_id, "cluster_status": status}}
        if status == "resuming":
            return {"status": "pending", "message": "Cluster is resuming",
...
prompt = (
    "Create an operator that resumes a paused Amazon Redshift cluster and polls every 15 seconds "
    "until it reaches 'available' status. If already available, return success immediately. "
...
install_docs = """# AWSRedshiftResumeCluster - Install Guide
...
guide_docs = """# AWSRedshiftResumeCluster - Operator Guide

## What it does

Resumes a paused Amazon Redshift cluster and polls every 15 seconds until it reaches 'available'
status. If the cluster is already available, returns success immediately without calling
resume_cluster. Equivalent to Airflow's RedshiftResumeClusterOperator.

Note: Only ra3 and dc2.large node type clusters support pause/resume.
...
|-----------------------|----------|---------------------------------------------------------|
| cluster_identifier    | Yes      | Cluster to resume                                       |
...
- Does not work on dc2.8xlarge or ds2 node types (AWS limitation on pause/resume)
- Does not start or create a cluster — use AWSRedshiftCreateCluster for that
"""

description = """
Resumes a paused Amazon Redshift cluster and polls until 'available'. If already available, returns
success immediately. Equivalent to Airflow's RedshiftResumeClusterOperator. Required:
...
metadata = {
    ...
    "tags": ["redshift", "cluster", "resume", "aws"],
    "airflow_equivalent": "RedshiftResumeClusterOperator"
}
...
Resuming restarts the cluster's compute nodes and restores it to `available` state. The cluster must be in `paused` state. Resuming typically takes 5-10 minutes. All data is preserved from before the pause. Billing resumes immediately when the resume operation is initiated, not when the cluster reaches available state.
"""
