...
codeblock = {"main.py": '''"""
AWS SageMaker Stop Notebook Instance Operator

Stops a running SageMaker notebook instance and polls until Stopped. Async.
...
def initialize(least_action_task_object):
    ...
    try:
        ...
        log_info("task", "initialize", "start",
                 f"Initializing AWSSageMakerStopNotebookInstance for task: {task_laui}")
...
def run(least_action_task_object, client):
    """
    Stops a running SageMaker notebook instance.

    Payload fields:
        notebook_instance_name  (str, required)  -- name of the notebook instance to stop
    ...
    try:
        ...
        log_info("task", "run", "stopping_notebook_instance",
                 f"Stopping notebook instance: {notebook_instance_name}")
        # stop_notebook_instance is idempotent — calling it on an already-stopped instance returns without error
        client.stop_notebook_instance(NotebookInstanceName=notebook_instance_name)
        log_info("task", "run", "stop_requested",
                 f"Stop request sent for notebook instance: {notebook_instance_name}")
...
def check_completion(least_action_task_object, client, run_details):
    """Poll describe_notebook_instance until Stopped or Failed."""
    ...
    try:
        ...
        if final_status == "success":
            ...
            log_info("task", "finish", "operation_summary",
                     f"Notebook instance {output.get('notebook_instance_name')} stopped. "
                     f"Compute billing halted — only EBS storage charges continue.")
...
prompt = (
    "Stops a running SageMaker notebook instance and polls until Stopped. "
    "Provide notebook_instance_name. "
    "Idempotent — calling on an already-stopped instance returns without error. "
    "Async — polls describe_notebook_instance until Stopped or Failed."
)
...
Stops a running SageMaker notebook instance and polls until it reaches the Stopped state. Stopping halts compute billing immediately — only EBS storage charges continue while the instance is stopped. All notebook files and data on the EBS volume are fully preserved. The stop call is idempotent — calling it on an already-stopped instance returns without error. This operator is async.
...
| Field                  | Required | Description                                         |
|------------------------|----------|-----------------------------------------------------|
| notebook_instance_name | Yes      | Name of the notebook instance to stop               |
...
## Output (on success)
    ...
    {
      "notebook_instance_name": "my-dev-notebook",
      "notebook_instance_status": "Stopped"
    }

| Field                    | Description                                              |
|--------------------------|----------------------------------------------------------|
| notebook_instance_name   | Name of the stopped notebook instance                    |
| notebook_instance_status | Final status — Stopped on success                        |

---

## Scenarios and Edge Cases

Instance already stopped (idempotent):
  If the instance is already in Stopped state, the stop_notebook_instance API call returns without error and the operator polls until it confirms Stopped status. Safe to call in scheduled pipelines without pre-checking status.

Instance in Pending state (may need to wait):
  If the instance is in a transient Pending or Updating state, the stop call may fail or succeed — behavior depends on the transition. If stop fails, retry after the instance reaches a stable state.

EBS data preserved across stop/start:
  All files on the EBS volume are fully preserved between stop and start cycles. There is no risk of data loss from stopping a notebook instance.

---

## What this operator does NOT do

- Does not delete the notebook instance — use AWSSageMakerDeleteNotebookInstance for permanent deletion.
- Does not save notebooks to S3 automatically before stopping — unsaved notebook state in the browser may be lost if not saved first.
"""

description = (
    "Stops a running SageMaker notebook instance and polls asynchronously until Stopped. "
    "Halts compute billing while preserving all EBS volume data."
)
...
metadata = {
    "service": "SageMaker",
    "category": "ML",
    "tags": ["sagemaker", "notebook", "stop", "aws"],
    "airflow_equivalent": "SageMakerStopNotebookInstanceOperator",
}
...
Stopping a notebook instance halts compute billing immediately — only EBS storage charges ($0.10/GB-month) continue after stop.
All data on the EBS volume is fully preserved between stop/start cycles — there is no risk of data loss from stopping.
The stop operation typically takes 1-3 minutes.
stop_notebook_instance is idempotent — calling it on an already-Stopped or Stopping instance returns without error, making it safe to use in scheduled pipelines without pre-checking status.
Use this operator in scheduled nightly pipelines to stop development instances outside business hours to reduce costs.
Pair with AWSSageMakerStartNotebookInstance to create start/stop scheduling workflows.
"""
