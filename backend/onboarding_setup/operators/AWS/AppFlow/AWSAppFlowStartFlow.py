...
def run(least_action_task_object, client):
    try:
        ...
        log_info("task", "run", "starting_flow", f"Starting AppFlow flow '{flow_name}'")
...
def check_completion(least_action_task_object, client, run_details):
    if run_details.get("status") == "failed":
        return {
            "status": "failed",
            "message": "AppFlow flow execution failed",
            "output": run_details.get("result", {})
        }
    log_info("task", "check_completion", "sync_complete",
             "AWSAppFlowStartFlow is synchronous - already complete")
    return {
        "status": run_details.get("status", "success"),
        "message": "AppFlow flow execution completed",
        "output": run_details.get("result", {})
    }
...
prompt = (
    "Create an operator that triggers an Amazon AppFlow flow by name and polls every 15 seconds "
    "until the execution reaches a terminal status (Successful, Error, or Canceled). "
...
install_docs = """# AWSAppFlowStartFlow - Install Guide
...
guide_docs = """# AWSAppFlowStartFlow - Operator Guide

## What it does

Triggers an Amazon AppFlow flow by name and polls every 15 seconds until the execution
reaches a terminal status: Successful, Error, or Canceled. Equivalent to Airflow's
AppflowRunOperator.
...
- Does not support scheduled or event-triggered flows - use on-demand flows only
"""

description = """
Triggers an Amazon AppFlow flow by name and polls every 15 seconds until the execution
reaches a terminal status (Successful, Error, or Canceled). Equivalent to Airflow's
AppflowRunOperator. Required: flow_name. Optional: poll_interval_seconds (default 15).
Auth: explicit keys, assume IAM role, or default chain. Returns flow_name, execution_id,
execution_status, records_processed, and bytes_processed on completion.
"""
...
publisher_notes = (
    "AppFlow flows must be configured as on-demand trigger type - scheduled flows cannot be "
    "started via start_flow API. The operator polls using describe_flow_execution_records which "
    "returns most-recent-first - execution_id is used to match the correct run. If another "
    "execution starts concurrently, the polling may pick up the wrong one."
)

metadata = {
    "service": "AppFlow",
    "category": "Integration",
    "tags": ["appflow", "etl", "integration", "aws"],
    "airflow_equivalent": "AppflowRunOperator"
}
