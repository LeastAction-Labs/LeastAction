...
def run(least_action_task_object, client):
    try:
        ...
        log_info("task", "run", "starting_full_flow",
                 f"Starting full execution of AppFlow flow '{flow_name}'")
...
def check_completion(least_action_task_object, client, run_details):
    if run_details.get("status") == "failed":
        return {
            "status": "failed",
            "message": "AppFlow full flow execution failed",
            "output": run_details.get("result", {})
        }
    log_info("task", "check_completion", "sync_complete",
             "AWSAppFlowStartFullFlow is synchronous - already complete")
    return {
        "status": run_details.get("status", "success"),
        "message": "AppFlow full flow execution completed",
        "output": run_details.get("result", {})
    }
...
prompt = (
    "Create an operator that triggers a full Amazon AppFlow flow execution (ignoring any "
    "incremental or scheduled state) and polls every 15 seconds until the execution reaches "
    "a terminal status (Successful, Error, or Canceled). "
    "Required payload: flow_name. Optional: poll_interval_seconds (default 15). "
...
install_docs = """# AWSAppFlowStartFullFlow - Install Guide
...
guide_docs = """# AWSAppFlowStartFullFlow - Operator Guide

## What it does

Triggers a full Amazon AppFlow flow execution, ignoring any incremental bookmark or scheduled
state, and polls every 15 seconds until the execution reaches a terminal status (Successful,
Error, or Canceled). Equivalent to Airflow's AppflowRunFullOperator.
...
- Does not create or configure flows (use AWS Console or CloudFormation)
- Does not apply incremental filters - always runs a full extraction
"""

description = """
Triggers a full Amazon AppFlow flow execution (ignoring incremental state) and polls every
15 seconds until the execution reaches a terminal status (Successful, Error, or Canceled).
Equivalent to Airflow's AppflowRunFullOperator. Required: flow_name. Optional:
poll_interval_seconds (default 15). Auth: explicit keys, assume IAM role, or default chain.
Returns flow_name, execution_id, execution_status, records_processed, and bytes_processed.
"""
...
publisher_notes = (
    "Same as StartFlow but explicitly resets incremental state - useful for full historical reloads. "
    "Note that this does NOT provide a parameter to AWS to force full load; it simply starts the flow "
    "which must be configured with a full-transfer setting on the source connector."
)

metadata = {
    "service": "AppFlow",
    "category": "Integration",
    "tags": ["appflow", "etl", "full-load", "aws"],
    "airflow_equivalent": "AppflowRunFullOperator"
}
