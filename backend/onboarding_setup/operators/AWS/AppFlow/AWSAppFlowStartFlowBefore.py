...
def run(least_action_task_object, client):
    try:
        ...
        cutoff_time_str = payload.get("cutoff_time")

        missing = []
        if not flow_name:
            missing.append("flow_name")
        if not cutoff_time_str:
            missing.append("cutoff_time")
        if missing:
        ...
        cutoff_time = datetime.fromisoformat(cutoff_time_str.replace("Z", "+00:00"))
        poll_interval = int(payload.get("poll_interval_seconds", 15))

        log_info("task", "run", "starting_flow_before",
                 f"Starting AppFlow flow '{flow_name}', cutoff={cutoff_time_str}")
        ...
        while True:
            ...
            if now >= cutoff_time:
                log_info("task", "run", "cutoff_reached",
                         f"Cutoff time {cutoff_time_str} reached before flow completed")
                return {
                    "execution_type": "sync",
                    "status": "success",
                    "result": {
                        ...
                        "execution_status": "CutoffReached",
                        "records_processed": 0,
                        "bytes_processed": 0,
                        "message": f"Cutoff time {cutoff_time_str} reached before flow completed"
                    }
                }
            ...
            if executions:
                ...
                log_info("task", "run", "polling_status",
                         f"Flow '{flow_name}' status: {exec_status}, records: {records_processed}")
...
def check_completion(least_action_task_object, client, run_details):
    if run_details.get("status") == "failed":
        return {
            "status": "failed",
            "message": "AppFlow flow-before execution failed",
            "output": run_details.get("result", {})
        }
    log_info("task", "check_completion", "sync_complete",
             "AWSAppFlowStartFlowBefore is synchronous - already complete")
    return {
        "status": run_details.get("status", "success"),
        "message": "AppFlow flow-before execution completed",
        "output": run_details.get("result", {})
    }
...
payload = {
    ...
    "cutoff_time": "2024-01-15T12:00:00Z",    # required, ISO8601 UTC
    # "poll_interval_seconds": 15,            # optional
}

prompt = (
    "Create an operator that triggers an Amazon AppFlow flow and polls until execution completes "
    "or the cutoff_time (ISO 8601 UTC) is reached. "
    "Required payload: flow_name, cutoff_time. Optional: poll_interval_seconds (default 15). "
    "If cutoff is reached first, return status:success with execution_status=CutoffReached. "
...
install_docs = """# AWSAppFlowStartFlowBefore - Install Guide
...
guide_docs = """# AWSAppFlowStartFlowBefore - Operator Guide

## What it does

Triggers an Amazon AppFlow flow and polls every 15 seconds until the execution completes
or until a specified cutoff_time is reached. If the cutoff time arrives before the flow
finishes, the operator returns success with execution_status=CutoffReached. Equivalent to
Airflow's AppflowRunBeforeOperator.
...
## Payload

    {
      ...
      "cutoff_time": "2024-01-15T12:00:00Z"
    }

| Field                 | Required | Description                                                  |
|-----------------------|----------|--------------------------------------------------------------|
| flow_name             | Yes      | Name of the AppFlow flow to trigger                          |
| cutoff_time           | Yes      | ISO 8601 UTC datetime - stop polling at or after this time   |
...
If cutoff is reached before completion, execution_status will be "CutoffReached".

---

## What this operator does NOT do

- Does not cancel the AppFlow execution when cutoff is reached - it just stops polling
- Does not create or configure flows
"""

description = """
Triggers an Amazon AppFlow flow and polls every 15 seconds until the execution completes
or the cutoff_time (ISO 8601 UTC) is reached. If cutoff is reached first, returns success
with execution_status=CutoffReached. Equivalent to Airflow's AppflowRunBeforeOperator.
Required: flow_name, cutoff_time. Optional: poll_interval_seconds (default 15).
Auth: explicit keys, assume IAM role, or default chain. Returns flow_name, execution_id,
execution_status, records_processed, and bytes_processed.
"""
...
publisher_notes = (
    "Returns CutoffReached (success with execution_status=CutoffReached) if no execution completes "
    "before cutoff_time. This is a silent-success edge case - the caller should check execution_status "
    "in the output to distinguish genuine success from cutoff."
)

metadata = {
    "service": "AppFlow",
    "category": "Integration",
    "tags": ["appflow", "etl", "cutoff", "aws"],
    "airflow_equivalent": "AppflowRunBeforeOperator"
}
