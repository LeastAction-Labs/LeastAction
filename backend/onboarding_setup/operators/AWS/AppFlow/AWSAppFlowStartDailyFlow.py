...
def run(least_action_task_object, client):
    try:
        ...
        logical_date_str = payload.get("logical_date")
        if logical_date_str:
            logical_date = datetime.fromisoformat(logical_date_str.replace("Z", "+00:00")).date()
        else:
            logical_date = datetime.now(timezone.utc).date()
        ...
        log_info("task", "run", "starting_daily_flow",
                 f"Starting daily AppFlow flow '{flow_name}' for date {logical_date}")
            ...
            for exec_rec in executions:
                ...
                if started_at:
                    ...
                    if started_dt >= today_start:
                        todays_exec = exec_rec
                        break

            if todays_exec:
                exec_status = todays_exec.get("executionStatus")
                exec_result = todays_exec.get("executionResult", {})
                ...
                log_info("task", "run", "polling_status",
                         f"Flow '{flow_name}' daily execution status: {exec_status}, "
                ...
                log_info("task", "run", "waiting_for_daily_exec",
                         f"No execution found for {logical_date} yet, waiting...")
...
def check_completion(least_action_task_object, client, run_details):
    if run_details.get("status") == "failed":
        return {
            "status": "failed",
            "message": "AppFlow daily flow execution failed",
            "output": run_details.get("result", {})
        }
    log_info("task", "check_completion", "sync_complete",
             "AWSAppFlowStartDailyFlow is synchronous - already complete")
    return {
        "status": run_details.get("status", "success"),
        "message": "AppFlow daily flow execution completed",
        "output": run_details.get("result", {})
    }
    ...
    try:
        ...
        if final_status == "success":
            ...
            log_info("task", "finish", "operation_summary",
                     f"Flow {output.get('flow_name')} date={output.get('logical_date')} "
...
payload = {
    "flow_name": "my-appflow-daily-flow",  # required
    # "logical_date": "2024-01-15T00:00:00Z",  # optional, ISO8601 UTC, default today
    # "poll_interval_seconds": 15,  # optional
}

prompt = (
    "Create an operator that triggers an Amazon AppFlow daily flow and polls until an execution "
    "that started on or after the logical_date (defaults to today UTC) completes. "
    "Required payload: flow_name. Optional: logical_date (ISO date, defaults to today UTC), "
    "poll_interval_seconds (default 15). "
    "Auth: Case 1 explicit keys, Case 2 assume IAM role via STS, Case 3 default chain. "
    "Return flow_name, execution_id, execution_status, records_processed, bytes_processed, logical_date. "
    "Catch all AWS errors and return them as status:failed - never raise."
)

install_docs = """# AWSAppFlowStartDailyFlow - Install Guide
...
guide_docs = """# AWSAppFlowStartDailyFlow - Operator Guide

## What it does

Triggers an Amazon AppFlow flow scoped to a daily run and polls every 15 seconds until an
execution that started on or after the logical_date (start of day UTC) completes. If no
logical_date is provided, defaults to today in UTC. Equivalent to Airflow's
...
## Payload
    ...
    {
      "flow_name": "my-appflow-daily-flow"
    }

| Field                 | Required | Description                                                         |
|-----------------------|----------|---------------------------------------------------------------------|
| flow_name             | Yes      | Name of the AppFlow flow to trigger                                 |
| logical_date          | No       | ISO date (e.g. 2024-01-15) - defaults to today UTC                 |
...
- Does not filter records to a single day - it scopes which execution to observe
- Does not create or configure flows
"""

description = """
Triggers an Amazon AppFlow flow and polls every 15 seconds until an execution that started
on or after the logical_date (start of day UTC) completes. Defaults to today if no date is
provided. Equivalent to Airflow's AppflowRunDailyOperator. Required: flow_name. Optional:
logical_date (ISO date), poll_interval_seconds (default 15). Auth: explicit keys, assume IAM
role, or default chain. Returns flow_name, execution_id, execution_status, records_processed,
bytes_processed, and logical_date.
"""
...
publisher_notes = (
    "Matches execution by start time >= logical_date. If no execution has started yet for that date, "
    "the operator will poll indefinitely - add a timeout in your pipeline. logical_date defaults to "
    "today UTC midnight."
)
...
metadata = {
    ...
    "tags": ["appflow", "etl", "daily", "aws"],
    "airflow_equivalent": "AppflowRunDailyOperator"
}
