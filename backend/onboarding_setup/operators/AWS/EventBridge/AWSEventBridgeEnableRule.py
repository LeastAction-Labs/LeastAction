...
codeblock = {"main.py": '''"""
AWS EventBridge Enable Rule Operator

Enables a disabled EventBridge rule. Synchronous.
...
def initialize(least_action_task_object):
    ...
    try:
        ...
        log_info(
            ...
            f"Initializing EventBridge enable rule operator for task: {task_laui}"
        )
...
def run(least_action_task_object, client):
    """
    Enable a disabled EventBridge rule using enable_rule.

    Payload fields:
        rule_name       (str, required)  -- name of the rule to enable
    ...
    try:
        ...
        log_info(
            ...
            f"Extracting enable rule configuration for task: {task_laui}"
        )
        ...
        log_info(
            "task", "run", "enabling_rule",
            f"Enabling EventBridge rule: {rule_name} | bus={payload.get('event_bus_name', 'default')}"
        )

        client.enable_rule(**kwargs)

        log_info("task", "run", "rule_enabled", f"Rule {rule_name} enabled successfully")
...
def check_completion(least_action_task_object, client, run_details):
    """
    EnableRule is synchronous — pass through run_details directly.
    ...
    if run_details.get("status") == "failed":
        ...
        return {
            "status": "failed",
            "message": f"EnableRule failed: {run_details.get('error')}",
            "output": run_details.get("result"),
        }

    result = run_details.get("result", {})
    log_info("task", "check_completion", "sync_complete",
             f"EnableRule completed — rule={result.get('rule_name')}, state={result.get('state')}")
    return {
        "status": "success",
        "message": f"EventBridge rule {result.get('rule_name')} is now ENABLED",
        "output": result,
    }
    ...
    try:
        ...
        if final_status == "success":
            ...
            log_info("task", "finish", "operation_summary",
                     f"Rule {output.get('rule_name')} enabled successfully — state={output.get('state')}")
        elif final_status == "failed":
            log_error("task", "finish", "operation_failed",
                      f"EnableRule operation failed: {completion_details.get('message')}")
        else:
...
prompt = (
    "Enable a disabled EventBridge rule via enable_rule. Payload: rule_name (required). "
    "Optional: event_bus_name. Idempotent — enabling an already-enabled rule succeeds silently. "
...
install_docs = """# AWSEventBridgeEnableRule — Install Guide
...
## AWS Permissions Required
    ...
    {"Effect": "Allow", "Action": ["events:EnableRule", "events:ListEventBuses"], "Resource": "*"}
"""

guide_docs = """# AWSEventBridgeEnableRule — Operator Guide

## What it does

Enables a disabled EventBridge rule. Synchronous — returns immediately. Idempotent — enabling
an already-enabled rule succeeds with no error. Use DisableRule to pause without deleting.
...
|----------------|----------|--------------------------------------------------|
| rule_name      | Yes      | Name of the rule to enable                       |
...
## Output (on success)
    ...
    {"rule_name": "my-rule", "state": "ENABLED"}
"""

description = """
Enables a disabled EventBridge rule via enable_rule. Synchronous — returns immediately.
Idempotent — enabling an already-enabled rule succeeds silently.
Auth: explicit keys first, then assume_iam_role via STS, then default credential chain.
...
metadata = {
    ...
    "tags": ["eventbridge", "rule", "enable", "aws"],
    "airflow_equivalent": "EventBridgeEnableRuleOperator"
}
