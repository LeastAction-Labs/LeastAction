...
def run(least_action_task_object, client):
    try:
        ...
        log_info("task", "run", "deleting_nodegroup",
                 f"Deleting EKS nodegroup '{nodegroup_name}' from cluster '{cluster_name}'")

        client.delete_nodegroup(clusterName=cluster_name, nodegroupName=nodegroup_name)

        log_info("task", "run", "delete_initiated",
                 f"Nodegroup '{nodegroup_name}' delete initiated, polling every 30s until gone...")
        ...
        while True:
            try:
                ...
                if e.response["Error"]["Code"] == "ResourceNotFoundException":
                    log_info("task", "run", "nodegroup_gone",
                             f"Nodegroup '{nodegroup_name}' no longer exists - deletion complete")
                    break
                raise

        log_info("task", "run", "delete_complete",
                 f"EKS nodegroup '{nodegroup_name}' successfully deleted")
...
def check_completion(least_action_task_object, client, run_details):
    log_info("task", "check_completion", "sync_complete",
             "EKS DeleteNodegroup is synchronous - already complete")
    return {
        "status": run_details.get("status", "success"),
        "message": "EKS nodegroup deletion completed",
...
prompt = (
    "Create an operator that deletes an EKS managed nodegroup and waits until it is fully deleted. "
    "Required payload fields: cluster_name, nodegroup_name. "
    "Auth: try IAM role via STS first; if unavailable, fall back to access keys from connection. "
    "After calling delete_nodegroup, poll describe_nodegroup every 30 seconds. "
    "When ResourceNotFoundException is raised, the nodegroup is gone - return status:success. "
    "Return cluster_name, nodegroup_name, and status=DELETED on success. "
    "Catch all AWS errors and return them as status:failed - never raise."
)

install_docs = """# AWSEKSDeleteNodegroup - Install Guide
...
guide_docs = """# AWSEKSDeleteNodegroup - Operator Guide

## What it does

Deletes an Amazon EKS managed nodegroup and polls synchronously every 30 seconds until the
nodegroup is fully deleted (ResourceNotFoundException confirms deletion).
...
| nodegroup_name | Yes      | Name of the nodegroup to delete          |
...
description = """
Deletes an Amazon EKS managed nodegroup and polls every 30 seconds until ResourceNotFoundException
confirms deletion is complete. Auth: IAM role via STS first, fallback to access keys. Returns
cluster_name, nodegroup_name, and status=DELETED on success.
"""
...
metadata = {
    "service": "EKS",
    "category": "Compute",
    "tags": ["eks", "kubernetes", "nodegroup", "delete", "aws"],
    "airflow_equivalent": "EksDeleteNodegroupOperator"
}
...
EC2 instances in the node group are terminated on deletion. Other node groups and Fargate profiles in the cluster are unaffected. Deletion completes when `describe_nodegroup` raises `ResourceNotFoundException`. Any EC2 Spot instances in the group will be interrupted and terminated.
"""
