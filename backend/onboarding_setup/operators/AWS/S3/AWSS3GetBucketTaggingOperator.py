...
def run(least_action_task_object, client):
    try:
        ...
        log_info("task", "run", "get_tagging_start", f"Getting tags for bucket: {bucket}")

        try:
            response = client.get_bucket_tagging(Bucket=bucket)
            tag_set = response.get("TagSet", [])
            tags = {tag["Key"]: tag["Value"] for tag in tag_set}
            log_info("task", "run", "get_tagging_success",
                     f"Retrieved {len(tags)} tag(s) from bucket: {bucket}")
        except ClientError as e:
            error_code = e.response.get("Error", {}).get("Code", "Unknown")
            if error_code == "NoSuchTagSet":
                log_info("task", "run", "no_tags", f"Bucket {bucket} has no tags")
                tags = {}
            else:
                raise
        ...
        return {
            "execution_type": "sync",
            "status": "success",
            "result": {"bucket": bucket, "tags": tags}
        }

    except ClientError as e:
        error_code = e.response.get("Error", {}).get("Code", "Unknown")
        error_msg = e.response.get("Error", {}).get("Message", str(e))
        log_error("task", "run", "get_tagging_failed", f"({error_code}) {error_msg}")
...
def check_completion(least_action_task_object, client, run_details):
    log_info("task", "check_completion", "sync_complete",
             "S3 get bucket tagging is synchronous - already complete")
    return {
    ...
    try:
        ...
        if status == "success":
            ...
            log_info("task", "finish", "summary",
                     f"Bucket: {output.get('bucket')}, Tags retrieved: {len(output.get('tags', {}))}")
        else:
...
prompt = "Get the tags on an S3 bucket. Provide the bucket name in the payload. Uses IAM role first, falls back to access keys in connection."

install_docs = """# AWSS3GetBucketTagging — Install Guide
...
guide_docs = """# AWSS3GetBucketTagging — Operator Guide

## What it does

Retrieves all tags from an S3 bucket and returns them as a flat key-value dict. Handles
NoSuchTagSet gracefully — returns an empty dict if the bucket has no tags, rather than failing.
...
|--------|----------|-----------------------------------|
| bucket | Yes      | Name of the S3 bucket to read tags from |
...
- tags is an empty dict {} if the bucket has no tags — not an error
"""

description = """
Reads the complete tag set from an S3 bucket and returns it as a flat key-value dictionary.
Handles NoSuchTagSet gracefully — untagged buckets return an empty dict rather than a failure,
making this safe to use without pre-checking tag existence. Ideal for tag-based compliance
audits, cost allocation workflows, and conditional pipeline branching. Auth: IAM role via STS
first, fallback to flat access keys in connection. Returns bucket name and tags dict on success.
"""
...
metadata = {
    "service": "S3",
    "category": "Storage",
    "tags": ["s3", "bucket", "tagging", "read", "aws"],
    "airflow_equivalent": "S3GetBucketTaggingOperator"
}
...
If the bucket has no tags, AWS returns a NoSuchTagSet error — this operator handles it gracefully and returns an empty tags dict rather than failing. This makes it safe to use in conditional pipelines where the existence of tags is unknown.
"""
