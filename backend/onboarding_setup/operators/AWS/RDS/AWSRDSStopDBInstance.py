...
codeblock = {"main.py": '''"""
AWS RDS Stop DB Instance Operator

Stops a running RDS DB instance. Supports optional snapshot on stop. Async â€” polls until stopped.
Auth priority: explicit keys â†’ assume IAM role â†’ default credential chain.
Execution is async â€” run() triggers the stop; check_completion() polls until stopped state.

Note: AWS automatically restarts stopped instances after 7 days.
No compute charges while stopped, only storage.
"""
...
def initialize(least_action_task_object):
    ...
    try:
        ...
        log_info(
            ...
            f"Initializing RDS stop DB instance operator for task: {task_laui}"
        )
...
def run(least_action_task_object, client):
    """
    Stop a running RDS DB instance using stop_db_instance.

    Payload fields:
        db_instance_identifier   (str, required)  -- DB instance identifier to stop
        db_snapshot_identifier   (str, optional)  -- create a snapshot before stopping
    ...
    try:
        ...
        log_info(
            ...
            f"Extracting stop DB instance configuration for task: {task_laui}"
        )
        ...
        stop_kwargs = {"DBInstanceIdentifier": db_instance_identifier}
        if db_snapshot_identifier:
            stop_kwargs["DBSnapshotIdentifier"] = db_snapshot_identifier
            log_info(
                "task", "run", "snapshot_on_stop",
                f"Will create snapshot {db_snapshot_identifier} before stopping"
            )

        log_info(
            "task", "run", "stopping_db_instance",
            f"Issuing stop_db_instance for: {db_instance_identifier}"
        )

        client.stop_db_instance(**stop_kwargs)

        log_info(
            "task", "run", "stop_issued",
            f"stop_db_instance call succeeded for {db_instance_identifier} â€” "
            f"instance is transitioning to stopped state asynchronously"
        )
...
def check_completion(least_action_task_object, client, run_details):
    """
    Poll describe_db_instances to determine whether the DB instance is stopped.
    ...
    try:
        ...
        if db_status == "stopped":
            log_info("task", "check_completion", "db_stopped",
                     f"DB instance {db_instance_identifier} is stopped")
            return {
                "status": "success",
                "message": f"DB instance {db_instance_identifier} is stopped",
                "output": {
                    "db_instance_identifier": db_instance_identifier,
        ...
        log_info("task", "check_completion", "db_still_stopping",
                 f"DB instance {db_instance_identifier} still stopping â€” current status: {db_status}")
        return {
            "status": "pending",
    ...
    try:
        ...
        if final_status == "success":
            ...
            log_info(
                ...
                f"DB instance {output.get('db_instance_identifier')} stopped â€” "
                f"status={output.get('db_instance_status')}"
            )
...
payload = {
    ...
    # "db_snapshot_identifier": "my-snapshot-before-stop"  # optional â€” create a snapshot before stopping
}

prompt = (
    "Stop a running RDS DB instance. Payload: db_instance_identifier (required), "
    "db_snapshot_identifier (optional â€” create snapshot before stopping). "
    "Call stop_db_instance and return immediately with status:pending (async). "
    "check_completion polls describe_db_instances until the instance reaches stopped state. "
    "Note: AWS automatically restarts stopped instances after 7 days. "
...
install_docs = """# AWSRDSStopDBInstance â€” Install Guide
...
## AWS Permissions Required
    ...
    {"Effect": "Allow", "Action": ["rds:StopDBInstance", "rds:DescribeDBInstances"], "Resource": "*"}
...
guide_docs = """# AWSRDSStopDBInstance â€” Operator Guide

## What it does

Stops a running RDS DB instance and returns immediately with status:pending (async).
check_completion polls describe_db_instances until the instance reaches stopped state.
...
|--------------------------|----------|----------------------------------------------------|
| db_instance_identifier   | Yes      | RDS DB instance identifier to stop                 |
| db_snapshot_identifier   | No       | Snapshot identifier to create before stopping      |
...
description = """
Stops a running RDS DB instance (async). Calls stop_db_instance and returns immediately.
Optionally creates a snapshot before stopping. check_completion polls describe_db_instances
until the instance reaches stopped state. Note: AWS automatically restarts stopped instances
after 7 days. Auth: explicit keys first, then assume_iam_role via STS, then default credential chain.
...
metadata = {
    "service": "RDS",
    "category": "Database",
    "tags": ["rds", "database", "stop", "aws"],
    "airflow_equivalent": "RdsStopDbOperator"
}
