...
codeblock = {"main.py": '''"""
AWS RDS Delete Event Subscription Operator

Deletes an RDS event subscription. Synchronous.
Auth priority: explicit keys â†’ assume IAM role â†’ default credential chain.
delete_event_subscription returns immediately â€” no polling needed.
"""
...
def initialize(least_action_task_object):
    ...
    try:
        ...
        log_info(
            ...
            f"Initializing RDS delete event subscription operator for task: {task_laui}"
        )
...
def run(least_action_task_object, client):
    """
    Delete an RDS event subscription using delete_event_subscription. Synchronous â€” returns immediately.

    Payload fields:
        subscription_name   (str, required)  -- name of the event subscription to delete
    ...
    try:
        ...
        log_info(
            ...
            f"Extracting delete event subscription configuration for task: {task_laui}"
        )
        ...
        if not subscription_name:
            log_error("task", "run", "missing_subscription_name",
                      "subscription_name is required in payload")
            return {
                "status": "failed",
                "execution_type": "sync",
                "result": None,
                "error": "subscription_name is required in payload",
            }

        log_info(
            "task", "run", "deleting_event_subscription",
            f"Issuing delete_event_subscription for: {subscription_name}"
        )

        response = client.delete_event_subscription(SubscriptionName=subscription_name)
        sub_arn = response["EventSubscription"].get("EventSubscriptionArn", "")

        log_info(
            "task", "run", "subscription_deleted",
            f"delete_event_subscription call succeeded for {subscription_name} â€” ARN: {sub_arn}"
        )
...
def check_completion(least_action_task_object, client, run_details):
    """
    Pass through run_details â€” delete_event_subscription is synchronous, no polling needed.

    Returns:
        dict with status, message, output passed through from run_details
    """
    log_info("task", "check_completion", "sync_passthrough",
             "DeleteEventSubscription is synchronous â€” passing through run_details")
    result = run_details.get("result") or {}
    return {
        "status": run_details.get("status", "success"),
        "message": "Synchronous delete_event_subscription completed",
        "output": result,
    }
    ...
    try:
        ...
        if final_status == "success":
            ...
            log_info(
                ...
                f"Event subscription {output.get('subscription_name')} deleted â€” "
                f"ARN: {output.get('event_subscription_arn')}"
            )
        elif final_status == "failed":
            log_error("task", "finish", "operation_failed",
                      f"Delete event subscription failed: {completion_details.get('message')}")
        else:
...
prompt = (
    "Delete an RDS event subscription. Payload: subscription_name (required). "
    "Call delete_event_subscription and return immediately with status:success (sync). "
...
install_docs = """# AWSRDSDeleteEventSubscription â€” Install Guide
...
## AWS Permissions Required
    ...
    {"Effect": "Allow", "Action": ["rds:DeleteEventSubscription", "rds:DescribeDBInstances"], "Resource": "*"}
...
guide_docs = """# AWSRDSDeleteEventSubscription â€” Operator Guide

## What it does

Deletes an RDS event subscription synchronously. Returns status:success immediately.
...
|---------------------|----------|----------------------------------------------|
| subscription_name   | Yes      | Name of the event subscription to delete     |
...
description = """
Deletes an RDS event subscription (sync). Calls delete_event_subscription and returns immediately.
check_completion passes through run_details â€” no polling needed.
Auth: explicit keys first, then assume_iam_role via STS, then default credential chain.
...
metadata = {
    "service": "RDS",
    "category": "Database",
    "tags": ["rds", "events", "delete", "aws"],
    "airflow_equivalent": "RdsDeleteEventSubscriptionOperator"
}
