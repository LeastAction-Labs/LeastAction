    ...
    try:
        ...
        if current_status == "RUNNING":
            log_info("task", "run", "already_running",
                     f"Application '{app_name}' is already RUNNING, skipping start")
            return {
        ...
        start_params = {"ApplicationName": app_name}
        if "run_configuration" in payload:
            start_params["RunConfiguration"] = payload["run_configuration"]

        log_info("task", "run", "starting_application",
                 f"Starting KDA V2 application '{app_name}'")
        client.start_application(**start_params)
    ...
    try:
        if run_details.get("status") in ("success", "failed"):
            return {
                ...
                "message": "KDA application start completed",
        ...
        if status == "STARTING":
            return {"status": "pending", "message": "Application is STARTING",
...
prompt = (
    "Create an operator that starts an Amazon Kinesis Data Analytics V2 application and polls "
    "until RUNNING. If already RUNNING, return success immediately. "
    "Required payload: application_name. "
    "Optional: run_configuration (dict), poll_interval_seconds (default 15). "
...
install_docs = """# AWSKinesisAnalyticsV2StartApplication - Install Guide
...
guide_docs = """# AWSKinesisAnalyticsV2StartApplication - Operator Guide

## What it does

Starts an Amazon Kinesis Data Analytics V2 application and polls every 15 seconds until it reaches
RUNNING status. If the application is already RUNNING, returns success immediately without calling
start_application. Equivalent to Airflow's KinesisAnalyticsV2StartApplicationOperator.
...
|-----------------------|----------|----------------------------------------------------------------------|
| application_name      | Yes      | Name of the KDA application to start                                |
| run_configuration     | No       | RunConfiguration dict (FlinkRunConfiguration, SqlRunConfigurations) |
...
- Does not create the application — use AWSKinesisAnalyticsV2CreateApplication first
- Does not configure application sources/sinks at start time (use application_configuration on create)
"""

description = """
Starts an Amazon Kinesis Data Analytics V2 application and polls every 15 seconds until RUNNING.
If already RUNNING, returns success immediately. Equivalent to Airflow's
KinesisAnalyticsV2StartApplicationOperator. Required: application_name. Optional:
run_configuration (dict), poll_interval_seconds (default 15). Auth: IAM role via STS first,
fallback to access keys. Returns application_name, application_arn, application_status.
"""
...
metadata = {
    ...
    "category": "Streaming",
    "tags": ["kinesis", "kda", "streaming", "flink", "aws", "start"],
    "airflow_equivalent": "KinesisAnalyticsV2StartApplicationOperator"
}
...
The application must be in READY state before starting — create it first with AWSKinesisAnalyticsV2CreateApplication. `run_configuration` specifies Flink job snapshot restore options; omit for a fresh start. The operator polls until the application reaches RUNNING state. Starting an already-running application returns an error.
"""
