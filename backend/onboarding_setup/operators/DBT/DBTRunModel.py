...
def _get_server_url(connection):
    ...
    if not url:
        raise ValueError("connection must contain 'dbt_server_url'")
    return url
...
def finish(least_action_task_object, client, completion_details, run_details):
    log_info('task', 'finish', 'cleanup', 'No resources to clean up')
'''}
...
description = "Runs a single dbt model by name via dbt-server /run-model. Used in full import mode where each model is an individual chained task."

prompt = "Runs a single dbt model by name via dbt-server /run-model. Used in full import mode where each model is an individual chained task."
