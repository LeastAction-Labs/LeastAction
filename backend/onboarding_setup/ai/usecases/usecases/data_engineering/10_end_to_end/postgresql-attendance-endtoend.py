...
description = (
    ...
    "NOTE: this is SQL-native staged modeling (staging -> intermediate -> fact -> mart layering, a "
    "dbt-style convention) executed as plain SQL via PostgresqlExecuteSQL — it does NOT invoke dbt. "
    "For real dbt invocation (DBTRunModel against a dbt project), see the dbt-sales-reporting usecase."
)

guide_docs = """\
# Badge Attendance Pipeline — Setup Guide

> End-to-end capstone (ingest -> stage -> intermediate -> fact -> mart -> reports). This is
> **SQL-native** staged modeling executed via `PostgresqlExecuteSQL` — the `stg_/int_/fct_/mart_`
> layering is a dbt-style naming convention, NOT dbt execution. For real dbt invocation see
> `dbt-sales-reporting`. The reporting half is `postgresql-attendance-reporting`.
...
metadata = {
    ...
    "category": "End-to-End",
    "tags": [
        "flavor:S+P", "lifecycle:end-to-end", "sql-native", "staged-modeling",
        "badge", "attendance", "postgresql", "pipeline",
        "sessions", "mart", "usecase",
    ],
}
