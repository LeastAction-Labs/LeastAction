...
*/
-- =============================================================================
-- STEP 00 — Ingest raw sales into fact_sales_daily
-- Creates the transaction-grain fact table and loads daily sales rows (one row per
-- sale: product, category, region/sub-region, store, channel, revenue, units, cost).
-- This is the raw source that the three cube stages below aggregate from.
-- =============================================================================
...
*/
-- =============================================================================
-- STEP 01 — Cube stage 1: aggregate to the dimension cube (fact_product_agg_daily_stage1)
-- Rolls fact_sales_daily up across product x category x region x sub-region x store
-- (a CUBE / GROUPING SETS aggregation) into one row per dim_key combination, with the
-- base metrics (revenue, profit, units_sold, cost) stored in a long
-- metric_key / metric_value shape that the later stages extend.
-- =============================================================================
...
*/
-- =============================================================================
-- STEP 02 — Stage 2 metrics: day-over-day + rolling 10-day (fact_product_agg_daily_stage2)
-- Copies stage 1, then appends derived metric rows per dim_key using window functions
-- ordered by date: DOD (vs the previous day) and rolling 10-day avg/std/min/max.
-- =============================================================================
...
*/
-- =============================================================================
-- STEP 03 — Final metrics: YOY / YTD / MTD / WOW / rank / penetration (fact_product_agg_daily)
-- Copies stage 2, then appends the remaining metric rows (year-over-year, period-to-date,
-- week-over-week, rank within group, and penetration/share) to reach the full 45-metric
-- cube that the two HTML report steps (04, 05) read from.
-- =============================================================================
...
metadata = {
    "tags": ["flavor:S+P", "lifecycle:transformation", "sql-native", "postgresql", "sql", "analytics", "etl", "reporting", "html", "dashboard", "sales", "metrics"],
