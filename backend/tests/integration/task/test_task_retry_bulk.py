...
async def test_retry(
    ...
    assert len(tasks_ready_to_run) == 500
    ...
    count = 1
    total_retries = 5
    # first iteration we will just check if 250 tasks with retry come in response of get tasks ready to run if retry interval passes and assert their retry_count == 1
    # second iteration we will do same asserts as 1st iteration but we will update payload lauis of first 125 tasks in tasks_ready_to_run at the end before starting cron
    # third iteration , we should get only 125 tasks in response of ready to retry and we will assert their retry_count and then we will assert retry_count of the tasks whose payload laui we changed their retry count should be reset to 0.
    # 4th iteration same as 1 just len will be 125
    ...
    while count <= total_retries:
        ...
        if count in [1, 2]:
            assert len(tasks_ready_to_run) == 250
        ...
        if count == 3:
            assert len(tasks_ready_to_run) == 125
            tasks_ready_to_run = sorted(tasks_ready_to_run, key=lambda task: task["laui"])
            for i in range(0, 125):
                assert tasks_ready_to_run[i]["laui"] == str(ctx.task_with_retry_lauis[i + 125])
                assert tasks_ready_to_run[i]["retry_number"] == count

            tasks = await test_database.items.find(
                {"_id": {"$in": ctx.task_with_retry_lauis[:125]}}, {"retry_number": 1, "state": 1}
            ).to_list(length=None)
            for task in tasks:
        ...
        if count == 2:
            # change payload lauis of 125 rerty tasks
            await test_database.items.update_many(
                {"_id": {"$in": ctx.task_with_retry_lauis[:125]}}, {"$set": {"payload": '"pass"'}}
            )
            # these tasks will pass check if their retry_number got reset to zero.
